# P08 — Mission statement, minus the buzzwords (deepseek-v4-pro)
...
- Duration: 44.5s  Meta: {"usage": {"prompt_tokens": 1650, "completion_tokens": 3365, "total_tokens": 5015, "prompt_tokens_details": {"cached_tokens": 1408}, "completion_tokens_details": {"reasoning_tokens": 2807}, "prompt_cache_hit_tokens": 1408, "prompt_cache_miss_tokens": 242}, "reasoning_len": 12813}
...
A mission statement gives the WHY (the challenge), the WHAT (the thing we build), and the SO WHAT (the benefit).

Facts about our product: existing Epicourier and the stack in the evidence block; audience is household cooks managing perishables; candidate distinctive detail is a receipt showing which pantry lots and unknown-expiry data caused a recommendation.

Write three candidate mission statements, five sentences each. Banned words: leverage, empower, seamless, revolutionize, cutting-edge, innovative, solution. Each candidate must contain one concrete detail a rival could not copy-paste, and one measurable M0 claim (metric, threshold, baseline).
```
