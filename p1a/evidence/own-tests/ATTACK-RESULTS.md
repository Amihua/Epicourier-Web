...
| `test_attack_rejects_zero_inventory_quantity` | Inventory quantity is zero. | Reject zero and require a finite positive value. | **FAIL** — zero is accepted without a `ValidationError`. Medium: invalid recommendation input. |
| `test_attack_rejects_negative_inventory_quantity` | Inventory quantity is negative (`-100`). | Reject negative values and require a finite positive value. | **FAIL** — the negative quantity is accepted without a `ValidationError`. Medium: invalid recommendation input. |
