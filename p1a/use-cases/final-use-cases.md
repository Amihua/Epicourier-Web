# Epicourier Project 1a — Final 20 Use Cases

This final set uses the Codex version as its baseline because its goal granularity gives the best fit with the repository test suite. Step 8 comparison then reconciles claims from Gemini, Claude, and qwen2.5 against product source. Nineteen Codex goals are retained directly; UC18 is broadened to incorporate the source-confirmed recipe-to-shopping-list behavior found by Gemini and Claude. The set remains exactly 20 use cases. Main success scenarios contain only happy-path behavior; alternatives and failures are recorded as extensions. Source references use repository-relative file paths and line numbers.
...
| Part | Content |
|---|---|
| **Name** | Create and populate shopping list |
| **Primary actor** | User |
| **Stakeholders & interests** | User: organize groceries in a named list and add selected recipe ingredients without retyping them. Platform: store lists and items under the correct authenticated owner and retain catalogue links needed for later inventory transfer. |
| **Preconditions** | User is authenticated; to add from a recipe, the user is viewing recipe details. |
| **Trigger** | User chooses to create a shopping list or add recipe ingredients to a list. |
| **Main success scenario** | 1. User starts from a recipe and selects the ingredients to buy. 2. System loads shopping lists owned by the user. 3. User selects an existing target list. 4. User confirms the selected ingredients. 5. System appends the ingredients and quantities to the target list. 6. System confirms success and the populated list remains available for shopping. |
| **Extensions** | 2a: Shopping lists cannot be loaded → system records the failure and ends loading without a visible error (`web/src/components/shopping/AddToShoppingListModal.tsx:57-75`). 3a: User needs a new list → user optionally supplies a name; when blank, system uses “Shopping for <recipe>”, creates the owned list, and then adds the selected ingredients (`web/src/components/shopping/AddToShoppingListModal.tsx:121-157`; `web/src/app/api/shopping-lists/route.ts`). 4a: User deselects ingredients already on hand → system adds only the selected subset (`web/src/components/shopping/AddToShoppingListModal.tsx:78-104`). 5a: An item name is blank → API rejects it (`web/src/app/api/shopping-lists/[id]/items/route.ts:38-44`). 5b: No catalogue ingredient matches a manually entered item → system stores it by name only, so it is not eligible for later inventory transfer (`web/src/app/api/shopping-lists/[id]/items/route.ts:53-65`). |
| **Postconditions** | An owned shopping list exists and contains the selected recipe ingredients; matched items retain catalogue links for later inventory transfer. |
